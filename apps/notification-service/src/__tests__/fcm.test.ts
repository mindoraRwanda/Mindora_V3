...
const mocks = vi.hoisted(() => {
  ...
  return { mockSend, mockInitializeApp, mockCert, mockGetMessaging, mockReadFileSync }
})
...
describe('initFirebase', () => {
  ...
  it('initializes using inline JSON when value starts with {', async () => {
    process.env.FIREBASE_SERVICE_ACCOUNT_JSON = '{"type":"service_account","project_id":"test-proj"}'
    mocks.mockInitializeApp.mockReturnValue({ name: 'mock-app' })
    const { initFirebase } = await import('../fcm.js')
    initFirebase()
    expect(mocks.mockCert).toHaveBeenCalledWith({ type: 'service_account', project_id: 'test-proj' })
  ...
  it('reads credentials from file path when value does not start with {', async () => {
    process.env.FIREBASE_SERVICE_ACCOUNT_JSON = '/etc/secrets/service-account.json'
    mocks.mockReadFileSync.mockReturnValue('{"type":"service_account","from":"file"}')
    mocks.mockInitializeApp.mockReturnValue({ name: 'mock-app' })
    const { initFirebase } = await import('../fcm.js')
    initFirebase()
    expect(mocks.mockReadFileSync).toHaveBeenCalledWith('/etc/secrets/service-account.json', 'utf8')
    expect(mocks.mockCert).toHaveBeenCalledWith({ type: 'service_account', from: 'file' })
    expect(mocks.mockInitializeApp).toHaveBeenCalledOnce()
  })
})
...
describe('sendPushNotification', () => {
  ...
  it('warns and returns early when Firebase is not initialized', async () => {
    ...
    expect(consoleSpy).toHaveBeenCalledWith(expect.stringContaining('Firebase not initialized'))
  ...
  it('warns when no FCM token is present', async () => {
    ...
    expect(consoleSpy).toHaveBeenCalledWith(expect.stringContaining('No FCM token'))
  })
  ...
  it('throws FatalNotificationError for messaging/invalid-registration-token', async () => {
    process.env.FIREBASE_SERVICE_ACCOUNT_JSON = '{"type":"service_account"}'
    mocks.mockInitializeApp.mockReturnValue({ name: 'app' })
    mocks.mockSend.mockRejectedValue({ code: 'messaging/invalid-registration-token' })
    vi.stubGlobal('fetch', vi.fn().mockResolvedValue({
      ok: true,
      json: () => Promise.resolve({ fcmToken: 'stale-token' }),
    }))
    vi.spyOn(console, 'error').mockImplementation(() => {})
    const { initFirebase, sendPushNotification, FatalNotificationError } = await import('../fcm.js')
    initFirebase()
    await expect(sendPushNotification('user-1', 'T', 'B')).rejects.toBeInstanceOf(FatalNotificationError)
  })

  it('throws FatalNotificationError for messaging/registration-token-not-registered', async () => {
    process.env.FIREBASE_SERVICE_ACCOUNT_JSON = '{"type":"service_account"}'
    mocks.mockInitializeApp.mockReturnValue({ name: 'app' })
    mocks.mockSend.mockRejectedValue({ code: 'messaging/registration-token-not-registered' })
    vi.stubGlobal('fetch', vi.fn().mockResolvedValue({
      ok: true,
  ...
  it('re-throws transient FCM errors without wrapping', async () => {
    ...
    const { initFirebase, sendPushNotification } = await import('../fcm.js')
    initFirebase()
    await expect(sendPushNotification('user-1', 'T', 'B')).rejects.toBe(transientError)
  })
})
