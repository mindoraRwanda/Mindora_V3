...
const mocks = vi.hoisted(() => {
  ...
  return {
    mockSendToQueue, mockAssertQueue, mockClose, mockConsume,
    mockAck, mockNack, mockChannel, mockCreateChannel, mockConnection,
    mockConnect, mockSubscribeToExchange,
  }
})
...
describe('subscribeWithRetry', () => {
  ...
  it('calls subscribeToExchange with the given exchange and queue', async () => {
    ...
    expect(mocks.mockSubscribeToExchange).toHaveBeenCalledWith(
      'mindora.test', 'test.queue', expect.any(Function)
    )
  })
  ...
  it('invokes the handler on delivery without queuing a retry on success', async () => {
    ...
    const wrappedHandler = mocks.mockSubscribeToExchange.mock.calls[0][2] as (p: unknown) => Promise<void>
  ...
  it('routes FatalNotificationError directly to DLQ without touching a retry queue', async () => {
    ...
    const dlqCall = mocks.mockSendToQueue.mock.calls.find(([q]) => q === 'mindora.notifications.dlq')
    const retryCall = mocks.mockSendToQueue.mock.calls.find(([q]) => String(q).includes('retry'))
  ...
  it('DLQ envelope includes failedAt and error fields', async () => {
    ...
    const fatalErr = new FatalNotificationError('Bad token', 'messaging/invalid-registration-token', 'u1')
    const handler = vi.fn().mockRejectedValue(fatalErr)
    await subscribeWithRetry('mindora.messages', 'q', handler)
    const wrapped = mocks.mockSubscribeToExchange.mock.calls[0][2] as (p: unknown) => Promise<void>
    vi.spyOn(console, 'error').mockImplementation(() => {})
    await wrapped({ x: 1 })

    const [, buf] = mocks.mockSendToQueue.mock.calls.find(([q]) => q === 'mindora.notifications.dlq')!
...
describe('setupRetryInfrastructure', () => {
  ...
  it('asserts DLQ, reprocess queue, and 4 retry queues', async () => {
    ...
    const asserted = mocks.mockAssertQueue.mock.calls.map(([name]) => name as string)
  ...
  it('nacks malformed (non-JSON) messages on the reprocess queue', async () => {
    ...
    mocks.mockConsume.mockImplementation((_q: string, cb: (msg: unknown) => Promise<void>) => {
      consumeCallback = cb
  ...
  it('routes envelope with attempt >= MAX_RETRIES to DLQ via reprocess consumer', async () => {
    const { subscribeWithRetry, setupRetryInfrastructure } = await import('../retry.js')

    const failingHandler = vi.fn().mockRejectedValue(new Error('persistent error'))
    // Register handler so reprocess consumer can look it up by exchange
    mocks.mockSubscribeToExchange.mockResolvedValue(undefined)
    await subscribeWithRetry('mindora.appointments', 'notif.queue', failingHandler)
    vi.clearAllMocks()
    ...
    const envelope = { exchange: 'mindora.appointments', payload: { data: 'test' }, attempt: 4 }
  ...
  it('acks the reprocess message before scheduling the next retry', async () => {
    ...
    const failingHandler = vi.fn().mockRejectedValue(new Error('still failing'))
    mocks.mockSubscribeToExchange.mockResolvedValue(undefined)
    await subscribeWithRetry('mindora.appointments', 'notif.queue', failingHandler)
    vi.clearAllMocks()
    ...
    const envelope = { exchange: 'mindora.appointments', payload: { x: 1 }, attempt: 1 }
    const msg = { content: Buffer.from(JSON.stringify(envelope)) }
    vi.spyOn(console, 'warn').mockImplementation(() => {})
    await consumeCallback!(msg)
