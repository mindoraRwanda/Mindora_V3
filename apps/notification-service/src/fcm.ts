...
async function getFcmToken(userId: string): Promise<string | null> {
  ...
  try {
    ...
    if (!res.ok) {
      console.warn(`[fcm] User Service returned ${res.status} for user ${userId} — no FCM token`);
      return null;
    }
...
export async function sendPushNotification(
  ...
  if (!app) {
    console.warn(`[fcm] Firebase not initialized — skipping push to user ${userId}`);
    return;
  }
  ...
  try {
    ...
    if (code && FATAL_FCM_CODES.has(code)) {
      ...
      throw new FatalNotificationError(`FCM ${code} for user ${userId}`, code, userId);
    }
