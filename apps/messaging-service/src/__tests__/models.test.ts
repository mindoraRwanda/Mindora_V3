...
beforeAll(async () => {
  await mongoose.connect(
    'mongodb://localhost:27017/mindora_messaging_models_test'
  );
});
...
describe('Conversation model', () => {
  ...
  it('rejects a conversation with no participants', async () => {
    await expect(Conversation.create({ participants: [] })).rejects.toThrow();
  });

  it('rejects a conversation with missing participants field', async () => {
    await expect(Conversation.create({})).rejects.toThrow();
  });
