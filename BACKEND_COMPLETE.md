...
Each backend service manages its own dedicated database. Every
PostgreSQL-backed service owns its own Prisma schema and migrations
...
## seed.ts files (post database-separation)

**Update (2026-07-30):** partially fixed, partially still true, and one claim
below was simply wrong:

- `apps/user-service/src/seed.ts` and `apps/auth-service/src/seed.ts` now
  import from each service's own local Prisma client (`./lib/prisma.js`) —
  confirmed working, both run successfully today (`npm run db:seed:profiles`,
  `npm run seed -w @mindora/auth-service`).
- `apps/appointment-service/src/seed.ts` and
  `apps/mood-tracking-service/src/seed.ts` **still** import `@prisma/client`
  directly rather than their own local generated client — the original
  concern in this note stands for these two specifically. Not verified
  whether they currently run successfully or not; treat as suspect until
  someone confirms.
- **"Auth Service has no `seed.ts` file" was incorrect** — it exists
  (`apps/auth-service/src/seed.ts`) and seeds the 30 fixed-UUID `THERAPIST`
  auth accounts (dummy password, not a real login — see the root `README.md`
  Known Issues section for what this does and does not cover).

These are dev-only seeding scripts, not used in any request path or test
suite. Not a production concern.
...
- avatarUrl: always returns null — PatientProfile and TherapistProfile schemas
  ...
  **Update (2026-07-30):** `TherapistProfile` now has a `photoUrl` column
  (added for the therapist-directory photo feature, seeded for 8 of the 30
  therapists — see `CHANGELOG.md`). That does **not** fix this specific
  `avatarUrl` field, though — `resolveAuthor()` in
  `community.routes.ts` only calls User Service's public user lookup, which
  returns `{ id, userName }` with no photo field, so post-author `avatarUrl`
  is still hardcoded null regardless of the schema change. Wiring that up
  would mean adding `photoUrl` to whatever endpoint `resolveAuthor()` calls
  and mapping it through — not done, still deferred, just no longer blocked
  by "the column doesn't exist" for the therapist side.
