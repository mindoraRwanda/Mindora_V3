...
Each backend service manages its own dedicated database. The shared
...
`apps/user-service/src/seed.ts`, `apps/appointment-service/src/seed.ts`, and
`apps/mood-tracking-service/src/seed.ts` import `@prisma/client` directly
rather than each service's own local generated client. This resolved to the
now-removed shared `@mindora/database` package's generated client (hoisted
to the monorepo root `node_modules`), which is no longer regenerated by
anything now that `packages/database` is deleted. These are dev-only seeding
scripts, not used in any request path or test suite — they will need
rewriting against each service's own local Prisma client
(`src/lib/prisma.ts`) before they can run again. Not a production concern.
Auth Service has no `seed.ts` file, despite earlier drafts of this note
assuming otherwise.
